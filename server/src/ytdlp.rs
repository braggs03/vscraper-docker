...
pub struct YtdlpClient {
    ...
    downloads: Arc<Mutex<HashMap<Url, (Status, DownloadOptions, Option<Sender<Signal>>)>>>,
    ytdlp_path: String,
}
...
impl YtdlpClient {
    ...
    pub async fn download_from_options(
        ...
        self.add_download_handler(&url, options, tx).await;
    ...
    async fn add_download_handler(&self, url: &Url, options: &DownloadOptions, tx: Sender<Signal>) -> Result<()> {
        ...
        self.insert_download_db(url, Status::Running, options).await;
    ...
    pub async fn cancel_download(&self, url: Url) -> Result<Status> {
        match self.downloads.lock().await.get(&url) {
            Some(download) => match download {
                (Status::Running, _, Some(tx)) => match tx.send(Signal::Cancel).await {
